...
- `lad research next` 必须把待判定证据复核、provider backlog、可自动执行的研究数据请求、机会雷达下钻目标，以及研究任务下一步命令汇总成一个优先级队列。机会雷达行动必须优先把下钻目标的证据缺口转成补主题新闻或继续研究命令，避免用户看完雷达后仍需手动挑命令。每项必须展示行动区域、面向用户的行动标题、为什么要做、来源 artifact 和可复制命令。这是默认的新手“下一步做什么”入口，不得包含买入、卖出、持有、仓位、目标价或收益预期。`lad research run-next --action N` 必须只能执行白名单自动动作，第一版支持待判定证据复核、机会雷达主题新闻刷新和雷达触发的研究任务刷新链；它不得把队列里的命令字符串作为任意 shell 命令执行。待判定证据复核只能记录证据方向并打印重新核验命令，不得生成投资判断。执行后必须打印补证据结果；只有补到证据、记录证据复核或命中有效缓存时才打印下一步核验命令，`no-data` 和 `failed` 不得继续推进。雷达触发的研究任务如果尚未进入研究队列，必须先写入本地研究候选，再执行研究链。
