...
- `lad research deepen` creates second-stage research packets from the research queue, writing them to the local SQLite `research_packets` table and `.alphadesk/research/research-packets-*.json` with candidate identity, evidence IDs, expanded evidence, cached data, data gaps, proxy mappings, and next verification actions. Beginner-facing output must translate each candidate into a question card with at least: the question the system asks for the user, why to look, the observation entrypoint, what would make the signal meaningful, and the next action.
...
- `lad research check --strict` is the agent/CI workbench readiness entrypoint. It must run gap filling, regenerate research packets, produce a beginner-readable brief, and write machine-readable `workbench-check-*.json`. The beginner brief must not be just symbols, proxy tickers, or generic conclusions; it must explain the research question, why to look, what would make the signal meaningful, and what to do next. In strict mode it must exit non-zero whenever the evidence, research-entrypoint, proxy-price, or data-gap gates fail.
