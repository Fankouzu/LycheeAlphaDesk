...
- `lad research data-requests` 是研究数据请求队列入口，必须读取每个任务最新备忘录中的 `next_data_requests`，展示请求文本、建议补数据命令、来源 memo artifact 和对应的下钻核验 artifact。基金/ETF 资料请求必须指向 `lad data guide fund` 和 `lad data set fund --from-file`；行情请求必须指向 `lad data pull market`；新闻请求必须指向 `lad data pull news`；适用的美股公告/财报请求必须指向 `lad data pull filings`；每条请求最后都必须提供 `lad research verify` 重新核验命令。暂未接入自动 provider 的数据请求必须明确显示需要人工补来源或等待插件接入，不得伪装成已经自动覆盖。该命令只能作为补证据队列，不得将数据请求解释成买卖清单。
