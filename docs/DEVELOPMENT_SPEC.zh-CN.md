...
- `lad` 打开 TUI，主界面第一个动作必须是 `今日市场发现`，第二个动作必须是 `研究工作台`，然后是 `待判定证据队列`、研究复核历史、单条证据复核历史、研究备忘录历史、研究数据请求，再进入手动股票代码钻取、数据健康检查、provider setup 指引、snapshot 和退出。
