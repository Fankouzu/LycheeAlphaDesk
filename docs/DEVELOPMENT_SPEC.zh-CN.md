...
- TUI 今日市场发现成功后，结果页必须继续提供可用 ↑/↓ 和 Enter 选择的 `进入研究工作台`、`查看下一步行动`、`查看研究数据请求` 后续入口，不能只停留在静态报告。
...
- `lad data pull benchmark --symbols QQQ` 必须读取 Yahoo Finance 公开 Spark 历史数据，写入三条 `benchmark_comparison`：主题入口 5 个交易日变化、SPY 基准 5 个交易日变化和相对差异，并在 `lad research verify` 中显示为单独的“基准比较核验”和证据板支持行。必须保留来源 URL/日期，并说明这是历史背景，不是预测或交易指令。
- 成分级广度边界：公开 QQQ 接口返回的持仓总数/前十大视图不得被当作完整的同一时点成分价格面板。`nasdaq_public` 的 NDX/NDXE 代理完整后，仍存在的上涨家数/下跌家数请求必须保留，但不得继续推荐重复执行 `lad data pull breadth`；应指向有授权的成分数据 provider，或带来源的人工指标录入。Nasdaq 授权数据必须由用户自行申请并配置权限。
...
- QQQ 新闻边界：QQQ 证据请求必须使用不带 `--symbols` 的市场主题查询 `Nasdaq-100 technology stocks QQQ`；公司股票仍使用代码级查询，执行器不得把 ETF 代码当成公司新闻实体。
...
- `lad data guide financials --symbol ... --market HK` 与 `lad data set financials --from-file ...` 是港股数字财务的人工核验入口。向导只能读取已缓存的公告，不发起新请求；如果发现匹配的官方财务公告，必须列出最多 5 条来源候选，并预填最新一条的标题、公告日期和 URL，但仍要求用户打开原文核对并填写数字。模板必须验证报表类型、报告期、申报/公告日期、币种、至少一项财务数值和原始 URL；导入必须写入 `financials.json` 及 `financials-import-*.json` 审计记录，并在匹配港股人工财务请求时写入 `manual_required` fulfillment、移出待办队列，随后仍必须重新下钻核验。
...
- `lad data guide ipo --market HK|CN --name ...` 与 `lad data set ipo --from-file ...` 提供人工核验的 IPO/打新事件入口，写入 `ipo-events.json` 和审计记录。每条事件必须保留市场、交易所、公告日、申购起止日、上市日、状态、价格区间（如有）、手数（如有）、账户资格说明、风险说明和来源 URL；不得确认申购资格、预测收益或执行申购。
...
- `lad research deepen` 从研究队列生成二阶段研究深挖包，写入本地 SQLite `research_packets` 表和 `.alphadesk/research/research-packets-*.json`，包含候选身份、证据 ID、可展开证据、已缓存数据、数据缺口、代理映射和下一步核验动作。深挖层必须先从候选池生成深挖包，再优先展示无数据缺口、可继续研究的任务，避免默认工作台被阻塞项占满。相关新闻选择必须先按研究主题相关性排序，再按时间排序，避免最新但离题的 symbol 新闻挡住主题证据。ETF、基金和指数类任务的主题新闻还必须命中股票、指数、ETF、交易所、成交、流动性等金融市场语境，不能因为同城或泛技术关键词就进入相关新闻。面向用户的输出必须是工作台任务卡，而不是课件式说明；至少包含：研究问题、入口、优先级、排序依据、证据状态、验证标准、当前动作和下一步队列。证据状态必须包含支持、反向、待判定和离题证据数量；只有反向、待判定或离题证据时，任务必须降级为先复核证据方向。
...
- `lad research fill-gaps` 根据研究队列和深挖包暴露的数据缺口，自动补齐可拉取的数据；支持缺失行情、缺失的 ticker 关联新闻、美股股票 SEC 公告、港股股票 HKEXnews 公告、A 股股票巨潮资讯公告、美股 SEC XBRL 财务快照、具备权限时的 A 股财务快照，以及无 symbol 主题的可审计代理映射行情。港股数字财务会显示为人工交接，不会猜测数值。通过既有主题和市场语境过滤的当前 ticker 关联新闻，可以替代已经脱离本地缓存的历史 discovery evidence ID 作为研究证据；原始缺失 ID 仍须保留在 packet 中供审计，但不得永久阻塞本来已经有当前可审计证据的研究路径。provider 虽返回新闻行、但没有任何一行通过该过滤时，只能记为“部分完成”：保留原始行供审计，写入 `unresolved_news_symbols`，不得把新闻证据标为已完成。行情补齐默认使用 `auto`，美股走 Alpha Vantage；港股/A 股优先走已配置的 Tushare 路由，未配置时走 Eastmoney 日 K，最后使用 Yahoo chart 兜底。缺少 symbol 的候选不得被静默改写为某个代码；系统只能生成带原因、置信度和证据 ID 的代理标的，并要求用户人工确认后再下钻。
...
- 同一个核验 artifact 中如果多个请求最终指向完全相同的动作路径，必须合并为一条，避免同一数据缺口因为表述不同反复出现在新手待办中。
...
- 基准比较请求：如果 QQQ 研究数据请求明确要求与更宽市场基准比较，生成的行情动作必须同时获取 `QQQ` 和 `SPY`，并在重新核验前把两个代码传给行情 provider；只要求 QQQ 的请求仍保持单代码行为。
- A 股财务请求必须使用 Tushare 后端的 `lad data pull financials` 路径；港股数字财务请求在数字 provider 接入前必须保留为 HKEX 年报/业绩公告人工交接。
...
- 预测评估必须把已经有未来实际行情的预测与最后值基线比较，报告样本数、MAE 和区间覆盖率；没有未来实际值时必须显示“暂无样本”，不能伪造评分。
- walk-forward 生成必须记录每个输入截点，并在用户请求多个窗口时写入多条预测记录；多个窗口必须复用同一个已加载模型运行时，评估只聚合存在未来实际行情的窗口。
