...
非交互式下一步入口是 `lychee research next`；如果队列中某一项是白名单自动动作，可用 `lychee research run-next --action N` 执行单项，也可用 `lychee research run-next --count N --no-force` 连续推进前 N 个安全动作。批量推进每执行一步都会重建队列，遇到 `no-data`、`failed`、人工交接或队列首项不变时会停止，避免重复消耗数据源。第一版支持待判定证据复核、可自动执行的研究数据请求、机会雷达下钻目标的主题新闻刷新和雷达触发的研究链执行；已完成的研究数据请求会写入可审计的 fulfillment 记录并离开待办队列，避免同一动作反复出现；只有记录证据复核、补到证据或命中有效缓存后才打印下一步核验命令，`no-data` / `failed` 不会继续推进。它不会把任意队列文本当 shell 命令运行。
