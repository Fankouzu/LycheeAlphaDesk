...
- Financial snapshots: SEC EDGAR XBRL `companyfacts` covers US issuers, and `.SH`/`.SZ` A-shares use the optional Tushare income/cash-flow path. Each snapshot preserves reporting period, revenue, net income, operating cash flow, provider identity, and official source URL; unavailable comparable periods stay empty rather than being invented. Hong Kong numeric financials remain a manual HKEX annual/results-document handoff, and announcement titles are never treated as numeric facts.
- `data pull financials` accepts mixed US and Mainland China symbols in one invocation and partitions them to SEC and Tushare. Hong Kong symbols in the same batch are reported as an unsupported automatic financial provider and cause a non-zero exit, while supported-market snapshots are still written; no numeric facts are fabricated.
