...
- `lad data pull volatility --symbols QQQ` downloads Cboe's public VXN historical CSV and writes three `volatility_metrics` rows: latest close, 20-trading-day change, and trailing-252-observation percentile. VXN must be described as the Cboe Nasdaq-100 30-day implied-volatility index and as risk context only; it must not be used as a market-breadth proxy or trading instruction. The `research_metrics` cache uses a 24-hour TTL, a one-hour `no_data` cooldown, and `--force` as the explicit override. A complete Cboe VXN metric set may clear a matching volatility-provider backlog item, but it must not clear market-breadth or entity-news gaps.
- `lad data pull breadth --symbols QQQ` downloads the public Nasdaq NDX and NDXE history responses and writes three `market_breadth` proxy rows: 20-trading-day cap-weighted change, equal-weight change, and their spread. NDXE is an auditable expansion proxy, not an advancer/decliner count or a licensed real-time feed; source URLs, dates, and the non-equivalence note must be retained. A complete `nasdaq_public` proxy set may clear the matching market-breadth provider backlog item, but must not clear entity-news gaps. It uses the same 24-hour research-metric TTL, one-hour `no_data` cooldown, and explicit `--force` override.
- `lad data pull benchmark --symbols QQQ` must read the public Yahoo Finance Spark history for QQQ and SPY, write three `benchmark_comparison` rows for the five-trading-day theme change, benchmark change, and relative difference, and expose them in `lad research verify` as a distinct `基准比较核验` check and evidence-board support lines. It must preserve the source URL/date and describe the values as historical context only, never as a forecast or trading instruction.
- Constituent breadth boundary: the public QQQ endpoint's holding count/top-holdings view must not be treated as a complete point-in-time constituent price panel. When the `nasdaq_public` NDX/NDXE proxy set is complete, a residual advancer/decliner request must remain visible but must no longer suggest repeating `lad data pull breadth`; it must point to an authorized constituent-data provider or a source-backed manual metric. Licensed Nasdaq data access is opt-in and requires the user's own entitlement.
- `lad data pull fund-metadata --symbols QQQ` reads Invesco's public detail and holdings endpoints, preserving the tracked index, expense ratio, holdings count/top holdings, as-of date, provider, and official product URL in `fund-metadata.json`. A complete source-backed QQQ row must suppress matching pending fund-metadata requests, but must not suppress unrelated breadth, news, or consistency requests. The endpoint requires only public request headers and no user API key; failures must remain auditable and must fall back to the manual guide path for unsupported funds.
- `lad data pull news` writes built-in Marketaux, Finnhub, NewsAPI, GDELT, or installed news-plugin events into the local live cache. Without `--symbols` it pulls market-level news; with `--symbols` it pulls symbol-level news. `--query` passes topic keywords for evidence strengthening around a research theme; topic queries must use a provider that supports topic news. It uses news-cache freshness by default; `--force` refreshes explicitly. The news cache must preserve existing rows and append deduplicated new rows so refreshes do not change the meaning of `news_001`-style evidence IDs. Installed plugins are discovered only through the Python `lychee_alphadesk.news_providers` entry-point group; `auto` may select only enabled plugins whose declared capability and required settings match the request. Plugin rows must remain `NewsEvent` records with timestamp, headline, and source URL. Plugin exceptions must be sanitized and allowed to fall back only in `auto`; an explicitly selected plugin must report its own failure and must not silently change source.
- QQQ news boundary: a QQQ evidence request must use a market-theme query (`Nasdaq-100 technology stocks QQQ`) without `--symbols`, while company symbols remain symbol-scoped. The request executor must not treat an ETF ticker as a company-news entity.
- `lad data pull news --symbols 0700.HK --provider tencent_official` reads the official Tencent Newsroom HTML and writes only dated article rows with an original Tencent URL, headline, and `0700.HK` symbol association. The provider is company-originated entity news, not HKEX filing data or independent financial confirmation. It must preserve the distinction in the row summary and provider display name; it may be offered after a Tencent topic-news query is exhausted, before manual source entry.
- `lad data pull filings` writes recent SEC EDGAR filings for US symbols, HKEXnews announcements for `.HK` symbols, and CNINFO announcements for `.SH` / `.SZ` China A-share stock symbols into the common local live cache. The HKEX path must resolve the official active-stock list before reading the issuer's announcement-title page, retain the original document URL/date/headline/symbol, and never present HKEX announcements as SEC documents. The CNINFO path must resolve the official stock list, query the public announcement endpoint with the stock code plus organization ID, retain the original PDF URL, China-local publication date, headline, and symbol, and must not imply use of the separately licensed data-service API.
...
- `lad data pull financials` writes SEC EDGAR XBRL `companyfacts` snapshots for US issuers and routes all-`.SH`/`.SZ` requests to Tushare `income`/`cashflow` snapshots. Both paths must retain period, revenue, net income, operating cash flow, provider, and source URL without inventing unavailable values; prior-year fields remain empty without comparable periods. Research packets, task detail, verification checks, and evidence boards must surface cached snapshots as auditable facts. Tushare 40203 must be reported as an interface entitlement/points gap, not a key error. `.HK` requests remain an explicit manual HKEX financial-document handoff.
- Mixed financial requests must be partitioned by symbol market: US and Mainland China symbols may be written through SEC and Tushare in one invocation, while Hong Kong symbols in the same batch must be reported as an unsupported automatic numeric provider and exit non-zero without preventing supported-market snapshots from being written.
- `lad data guide financials --symbol ... --market HK` and `lad data set financials --from-file ...` are the manual numeric-financial handoff for Hong Kong. The template must validate report type, period end, filing/announcement date, currency, at least one financial value, and the original URL; import must write `financials.json` plus a `financials-import-*.json` audit record, mark a matching Hong Kong financial request with a `manual_required` fulfillment, remove it from pending queues, and still require drilldown verification.
...
- `lad data health` checks live cache presence and row counts, then reports separate US, HK, and China A-share market-coverage rows from cached price symbols and the latest market warnings. A cache with rows but retained provider warnings must be shown as a warning rather than a pass, so partial or fallback data is not presented as healthy coverage. A Tushare `40203` entitlement warning must apply only to the affected market and must not imply an outage for another market.
- `lad data snapshot` writes a unified JSON snapshot from the live cache.
- A `lad research run` with successful provider responses but no topic-relevant news after filtering must report `partial`, not `completed`. It may keep drilldown verification so the rejected evidence and source gap remain auditable, but it must not offer LLM memo generation until relevant evidence exists.
- When a verification artifact records `topic_news_exhausted`, its news request must not retry the same provider query. It must offer a manually verified source handoff and create an `entity_news` provider backlog item so an auditable issuer, exchange, or licensed company-news plugin can replace the failed semantic coverage. That backlog must use `lad data set news`, never a generic metric command.
- Opportunity-radar follow-up titles must preserve the target display name and inferred asset type when they seed a research candidate. ETF and index targets must not be downgraded to stocks or trigger issuer-filings refreshes merely because a follow-up title changed from `下钻` to `继续研究`.
- An explicit `lad research verify --symbol` or `--name` must expand beyond the default workbench candidate limit when needed, matching `lad research run`. A command shown by a research task must not fail merely because that task is currently outside the first default candidates.
...
- `lad research run` is the data-refresh execution chain for one research task. It must select a task, refresh related prices/news/applicable US SEC filings, HKEXnews announcements, or CNINFO China A-share announcements, rerun the workbench check, print the updated research detail, and write a `research-run-*.json` audit artifact. US-only SEC XBRL financial snapshots remain separate from the HK/CN announcement paths. If the task evidence quality is missing, needs_review, or mixed, the execution chain must build a topic-news query from the research theme and pull one extra round of topic news. The artifact must include structured `assessment` with stage, consistency-review state, evidence reading, and next decision. It must not produce buy/sell advice; it only advances evidence collection and research state.
...
- Benchmark comparison requests: when a QQQ research data request explicitly asks for a broader-market benchmark comparison, the generated market action must collect both `QQQ` and `SPY`, and fulfillment must pass both symbols to the market provider before verification. A request that only asks for QQQ must remain single-symbol.
- A-share financial requests must use the Tushare-backed `lad data pull financials` route; Hong Kong numeric financial requests must remain a manual HKEX annual/results-document handoff until a numeric provider is available.
...
- `lad data pull macro --provider hkma|fred --series ...` writes the latest HKMA or FRED observations into `research-metrics.json`, preserving the series name, observation date, provider, and official source URL with a 24-hour research-metric cache. HKMA is limited to the documented `hibor.fixing`, `hibor`, and `honia` series; FRED requires the configured API key. These metrics must be included in the Today Discovery LLM context, but may only provide rate/liquidity context and must not become buy/sell advice.
- `lad data pull sector --markets US,HK,CN --days 60` computes 20-trading-day sector-proxy changes from public historical prices. US sectors use sector ETFs and Hong Kong/Mainland China use explicitly named ETF proxies; every row must preserve its proxy boundary and source URL with a 24-hour research-metric cache. The metrics may enter Today Discovery and research-task context, but must not be presented as a complete sector index, constituent breadth panel, or investment advice.
