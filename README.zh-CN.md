...
非交互式下一步入口是 `lychee research next`；如果队列中某一项是白名单自动动作，可用 `lychee research run-next --action N` 执行。第一版支持待判定证据复核、机会雷达下钻目标的主题新闻刷新和雷达触发的研究链执行；只有记录证据复核、补到证据或命中有效缓存后才打印下一步核验命令，`no-data` / `failed` 不会继续推进。它不会把任意队列文本当 shell 命令运行。
