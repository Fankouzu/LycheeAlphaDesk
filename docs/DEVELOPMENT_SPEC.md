...
- `lad research deepen` creates second-stage research packets from the research queue, writing them to the local SQLite `research_packets` table and `.alphadesk/research/research-packets-*.json` with candidate identity, evidence IDs, expanded evidence, cached data, data gaps, proxy mappings, and next verification actions. The deepen layer must build packets from a candidate pool first, then prioritize ready packets without data gaps so the default workbench is not filled by blocked items. User-facing output must be a workbench task card, not lesson-style prose, with at least: research question, entrypoint, priority, ranking reason, evidence status, key checks, and next-action queue.
