...
这个命令要求先通过 `lychee setup` 配置并激活 LLM provider。如果没有 LLM provider，Today Discovery 必须失败，不能静默生成 fallback 报告。第一版会在 LLM 配置检查通过后，把 starter 研究报告写入 `.alphadesk/data/discovery-today.json`。
...
1. 继续把第一版 `Today Discovery` TUI/CLI 的 LLM-required starter 报告扩展成 provider-backed 报告。
