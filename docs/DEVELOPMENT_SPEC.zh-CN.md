...
- `lad research next` 必须把待判定证据复核、provider backlog、可自动执行的研究数据请求、机会雷达下钻目标，以及研究任务下一步命令汇总成一个优先级队列。机会雷达行动必须优先把下钻目标的证据缺口转成补主题新闻或继续研究命令，避免用户看完雷达后仍需手动挑命令。每项必须展示行动区域、面向用户的行动标题、为什么要做、来源 artifact 和可复制命令。这是默认的新手“下一步做什么”入口，不得包含买入、卖出、持有、仓位、目标价或收益预期。
