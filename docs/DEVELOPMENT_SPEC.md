...
- `lad research next` must aggregate pending evidence reviews, provider backlog items, executable research data requests, and research-task next commands into one prioritized queue. Each item must show the action area, user-facing action title, why it matters, source artifact, and a copyable command. This is the default beginner-facing "what should I do next" entrypoint and must not contain buy/sell/hold, allocation, target-price, or position-sizing language.
