...
- `lad` opens the TUI with `今日市场发现` as the first action and `研究工作台` as the second action, followed by `待判定证据队列`, research review history, row-level evidence review history, research memo history, research data requests, manual symbol drilldown, data health, provider setup guidance, snapshots, and quit.
