...
def test_discover_radar_prints_local_opportunity_signals(
    ...
    assert "可下钻目标" in result.stdout
    assert "NVIDIA (NVDA)" in result.stdout
    assert "缺少该标的的主题新闻缓存" in result.stdout
...
def test_research_data_requests_command_lists_actionable_requests(tmp_path: Path) -> None:
    ...
    assert "lychee data pull market --symbols QQQ,SPY --provider auto --force" in (
        result.stdout
    )
