...
- `lad research detail` 是单条研究任务的非交互式详情入口，必须复用 TUI 研究详情的 core 渲染逻辑，输出“研究结果”、信号读数、证据矩阵、行情、相关新闻、公告/财报线索、数据缺口和可执行刷新命令。不得在 CLI 和 TUI 中维护两套不同口径的研究详情。
- `lad research run` 是单条研究任务的数据刷新执行链，必须选择任务、刷新相关行情/新闻/适用的美股公告、重新运行工作台自检、输出更新后的研究详情，并写入 `research-run-*.json` 审计记录。该命令不得给出买卖建议，只能推进证据收集和研究状态更新。
