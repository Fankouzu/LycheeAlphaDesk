...
- `lad research provider-backlog` 必须从同一套备忘录数据请求队列中提取暂未接入自动 provider 的人工来源请求，并分类为 provider/plugin backlog。每一项必须展示研究任务、市场、请求文本、数据领域、插件类型、当前覆盖缺口、候选来源形态、来源 memo artifact、对应下钻核验 artifact 和下一步数据接入动作。该命令只用于数据能力规划，不得把 provider 缺口解释成投资机会或交易建议。
