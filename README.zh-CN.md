...
它会把 fallback 研究报告写入 `.alphadesk/data/discovery-today.json`。报告覆盖美股、港股和 A 股的 starter themes 与关注候选，并且会明确标记当前尚未启用 live LLM/provider 综合分析。
...
1. 继续把第一版 `Today Discovery` TUI/CLI fallback 报告扩展成 provider-backed 报告。
