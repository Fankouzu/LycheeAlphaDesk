...
- 公司事件：SEC filings、HKEX 公告、巨潮资讯公告、财报事件、业绩指引、IPO/打新机会。
...
`research run` 会选择一条研究任务，刷新该任务相关的行情、新闻和适用的公司公告：美股股票走 SEC EDGAR，港股股票走 HKEXnews，A 股股票走巨潮资讯，然后重新运行工作台自检并输出更新后的 `研究任务面板`。已有的 SEC 或 Tushare 财务快照会进入同一证据板；缺失时任务面板会给出 `data pull financials` 的精确补齐命令。港股公告会作为官方文件通过核验，港股数字财务仍通过人工 HKEX/发行人报表模板进入，不会伪造数值。如果任务当前只有反向、待判定或离题新闻，执行链会额外用研究主题生成 `--query`，再拉取一轮主题新闻，帮助系统从“证据不相关”推进到“尝试补强证据”。每次执行会写入 `.alphadesk/research/research-run-*.json`，用于审计 agent 到底刷新了什么、返回多少数据、哪些动作失败或使用缓存；审计记录也会保存结构化 `assessment`，包括阶段、一致性核验状态、证据读数和下一步判断。
...
股票任务缺少财务快照时会继续保留数据缺口：美股指向 SEC XBRL，港股指向人工财务资料向导，A 股显示 Tushare 权限或人工核验边界；公告标题不会被当作财报数值。
...
- 成分级广度边界：当前 QQQ 公开基金接口提供持仓总数和前十大持仓视图，并不等于完整的、同一时点的成分价格面板。NDX/NDXE 代理缓存已经存在后，数据源缺口队列不得继续把 `data pull breadth` 当成真实上涨家数/下跌家数的解决方案；队列应改为提示接入有授权的成分数据 provider，或录入带来源、人工核验的广度指标。Nasdaq 的授权数据产品需要用户自己申请账户和数据权限，开源默认配置不会静默捆绑。
...
同一个核验 artifact 中如果多个请求最终指向完全相同的动作路径，系统会合并为一条待办，避免同一数据缺口因为表述不同反复出现在新手队列中。

新闻 provider 的自动路径按代码逐个检查覆盖情况：第一个来源只返回部分代码时，会继续为未覆盖代码尝试下一个来源，并在结果中保留混合来源和诊断。按代码查询的 NewsAPI 结果还会经过实体命中和金融语境过滤，避免把同名娱乐、生活内容直接写成个股证据。

如果 QQQ 请求明确要求与更宽市场基准比较，生成的行情动作会自动扩展为 `QQQ,SPY`，确保工作台同时收集主题标的和比较基准，而不是只刷新 QQQ。
...
模板必须填写报表类型、报告期结束日、申报/公告日期、币种、至少一项营收/净利润/经营现金流，以及原始 HKEX 或发行人 IR URL。导入会写入 `financials.json` 和 `financials-import-*.json` 审计记录；如果它对应一条港股人工财务待办，系统会将待办标记为已人工交接并移出队列，但仍要求重新下钻核验。
...
| 1 | `gdelt` | GDELT | 全球新闻和事件 | 不需要 API key | 无 | [GDELT data/API](https://www.gdeltproject.org/data.html) | 开放全球新闻回退源。AlphaDesk 会把已支持代码映射为实体并保留原文 URL；限流和主题相关性仍会明确显示为研究约束。 |
...
导入文件需要 `symbol,name,quantity,avg_cost,currency,asset_type,as_of` 字段，可选提供 `account_id`、`fees_paid`、`taxes_paid` 和 `corporate_action_note`。缺少账户、费用、税费或公司行动核对信息会记录为审计缺口，不会被系统猜测或补写；同一代码来自多个账户时只按数量合并用于只读估值，并保留“需人工核对”的审计缺口。导入的额外代码只会提示，不会悄悄加入目标组合。整个流程只读，不连接券商下单。
...
| 下一阶段 | 稳定 provider/plugin API、更多授权数据源、自动化港股数字财务 provider 和真实券商 CSV 验收覆盖。 |
