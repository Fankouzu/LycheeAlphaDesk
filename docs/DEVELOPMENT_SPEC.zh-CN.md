...
- `lad research verify` 是单条研究任务的下钻核验入口，必须核验行情、成交量、新闻、公告/财报和代理标的状态，并写入 `research-verification-*.json`。核验结果只能表达通过、待核验、阻塞或不适用；同时必须生成“支持证据 / 风险或反向待查 / 待补证据”三栏证据板。新闻和 discovery 证据不得只按数量进入支持证据，必须先做主题相关性核验；未命中研究任务关键词的 headline/summary 应进入风险或反向待查。一致性结论必须保守为待人工核验，直到后续有专门的一致性分析引擎。
