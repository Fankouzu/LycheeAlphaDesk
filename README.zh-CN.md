...
- LLM 分析：市场主题、受影响行业、相关公司或 ETF、证据摘要、风险提示和下一步应拉取的数据。
...
这个命令要求先通过 `lychee setup` 配置并激活 LLM provider。运行时会先检查/拉取市场级新闻缓存，再以 `stream: true` 调用已配置的 OpenAI-compatible `/chat/completions` 接口，解析模型返回的 JSON，并把 `llm-synthesized` 研究报告写入 `.alphadesk/data/discovery-today.json`。如果没有可用新闻 provider、没有 LLM provider、请求失败，或模型没有返回有效 JSON，Today Discovery 必须失败，不能静默生成 fallback 报告。默认 LLM 读超时为 180 秒。
